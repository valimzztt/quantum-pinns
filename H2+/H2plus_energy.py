...
def train(model, N_f, epochs):
    print(f"\n--- Training R=1 with N_f = {N_f} points ---")
    ...
    for epoch in range(epochs + 1):
        ...
        # Sampling [-5, 5] box is sufficient for training
        N = N_f
        r = torch.abs(torch.randn(N, 1, device=device) * 2.5)
        z = torch.randn(N, 1, device=device) * 3.0
        ...
        # Norm Penalty (Approximate to keep psi non-zero)
        ...
        if epoch % 1000 == 0:
            print(f"Epoch {epoch} | Loss: {loss.item():.5f} | Energy: {E_param.item():.5f}")
...
if __name__ == "__main__":
    ...
    # Train
    model_trained = train(model, N_f=5000, epochs=15000)
