...
# 2. Setup Plotting Grid (1 Row, 2 Columns)
fig, ax = plt.subplots(1, 2, figsize=(14, 6))

# --- Plot 1 Setup (Wavefunction) ---
...
prob_exact = u_exact**2 # This is |u(r)|^2. If you want |R(r)|^2, divide by r^2.
# Based on your previous code, you plotted u(r)^2. 
# Let's align with the benchmark which usually plots R(r)^2 or u(r)^2. 
# I will plot |u(r)|^2 (Radial Probability Density) as it is standard.
...
for width, layers, name in configs:
    ...
    # Store History
    # Assumes model.E_history is a list of floats populated during training
...
for name, history in energy_histories:
    # Scale x-axis based on length of history vs epochs
    # If you save every epoch: x = range(len)
    # If you save every 10 epochs: x = range(len) * 10
    epochs_axis = np.linspace(0, epochs, len(history))
    
    ax[1].plot(epochs_axis, history, linewidth=2, alpha=0.8, label=name)

# Add exact energy line to history plot
...
# Optional: Zoom in on the convergence area
...
# Add Energy Error Column for clarity
